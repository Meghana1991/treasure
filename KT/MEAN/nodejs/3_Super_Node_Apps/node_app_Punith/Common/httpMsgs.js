...
var common=require('../Common/settings');
...
exports.Verifytoken=function(req,res,callback){
   ...
   if (token!=null && token !="") {
    jwt.verify(token, common.secret, function(err, decoded) {    
    if (err) {
        console.log(err);
        ...
        res.end();   
            ...
            console.log("success");
            callback({ success: true, message: 'token authentication success.' });             
        }
    })
    }
    else
    {
        callback({ success: false, message: 'token authentication not found.' });  
    }    
}